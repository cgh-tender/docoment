...
const menusData = computed(() => {
  const cloneDeep1 = cloneDeep(usePermissionStore().routes)
})
/** 搜索（防抖） */
const handleSearch = debounce(() => {
  ...
  resultList.value = flatMenusData.filter((menu) =>
    keyword.value
      ? menu.meta?.title?.toLocaleLowerCase("chinese").includes(keyword.value.toLocaleLowerCase().trim())
      : false
  )
  // 默认选中搜索结果的第一项
...
const handleEnter = () => {
  ...
  if (!name) {
    message.warning("无法通过搜索进入该菜单，请为对应的路由设置唯一的 Name")
    return
  }
  try {
    router.push({ name })
  } catch {
    message.error("该菜单有必填的动态参数，无法通过搜索进入")
    return
  }
  handleClose()
}
...
<template>
  <a-modal
    ...
    append-to-body
  >
    <a-input ref="inputRef" v-model="keyword" @input="handleSearch" placeholder="搜索菜单" size="large" clearable>
      <template #prefix>
        <SvgIcon name="search" />
      </template>
    </a-input>
    <a-empty v-if="resultList.length === 0" description="暂无搜索结果" :image-size="100" />
    <template v-else>
      <p>搜索结果</p>
      <a-layout>
        <a-layout-content ref="scrollbarRef" max-height="40vh">
          <SearchResult
            ref="searchResultRef"
...
.search-modal__private {
  ...
  .a-modal__footer {
    ...
    padding: var(--a-modal-padding-primary);
  }
}
</style>
