...
<template>
  <div class="app-container">
    ...
    <h5>Select 示例</h5>
    <a-select :loading="loading" v-model="value" filterable>
      <a-option v-for="(item, index) in options" v-bind="item" :key="index" placeholder="请选择" />
    </a-select>
    <h5>Select V2 示例（如果数据量过多，可以选择该组件）</h5>
    <a-select-v2 :loading="loading" v-model="value" :options="options" filterable placeholder="请选择" />
  </div>
</template>
