...
import { h, nextTick, reactive, ref, UnwrapNestedRefs } from "vue"
...
const xFormOpt: UnwrapNestedRefs<{
  data: { password: string; username: string }
  titleWidth: string
  titleColon: boolean
  rules: {
    password: { validator: ({ itemValue }: { itemValue: any }) => Error; required: boolean }[]
    username: { validator: ({ itemValue }: { itemValue: any }) => Error; required: boolean }[]
  }
  loading: boolean
  items: (
    | { field: string; itemRender: { name: string; props: { placeholder: string } }; title: string }
    | {
        field: string
        itemRender: { name: string; props: { placeholder: string } }
        title: string
      }
    | {
        itemRender: {
          children: (
            | { events: { click: () => Promise<any> | undefined }; props: { content: string } }
            | {
                events: { click: () => void }
                props: { type: string; content: string; status: string }
              }
          )[]
          name: string
        }
  ...
  <div class="app-container">
    ...
    <vxe-grid ref="xGridDom" v-bind="xGridOpt">
      ...
      <template #row-operate="{ row }">
        <a-button link type="primary" @click="crudStore.onShowModal(row)">修改</a-button>

        <a-popconfirm ok-text="Yes" cancel-text="No" @confirm="crudStore.onDelete(row)">
          <template #title="{ row }">
            <TextContent>
              确定 <strong style="color: red"> 删除 </strong> 用户
              <strong style="color: #409eff"> ${row.username} </strong> ？
            </TextContent>
          </template>
          <a-button link type="danger">删除</a-button>
        </a-popconfirm>
      </template>
    </vxe-grid>
    <!-- 弹窗 -->
